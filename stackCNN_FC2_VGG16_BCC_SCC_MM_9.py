...
model_name = 'stackCNN_FC2_VGG16_BCC_SCC_MM_9'
result_folder = os.path.join(experiment_folder, model_name)
os.makedirs(result_folder, exist_ok=True)

# Set the paths to the train and test folders
# train_folder = "/data2/cmdir/home/anhnv/vvhieu/Hien/Hien_Data_Feature/train_vgg16_features/vgg16_features"
# test_folder = "/data2/cmdir/home/anhnv/vvhieu/Hien/Hien_Data_Feature/test_vgg16_features/vgg16_features"

train_folder = "/data2/cmdir/home/anhnv/vvhieu/Hien/Hien_Data_Feature/train_Hien_VGG16_FC_2_full"
test_folder = "/data2/cmdir/home/anhnv/vvhieu/Hien/Hien_Data_Feature/test_Hien_VGG16_FC_2_full"
...
train_images, train_labels, _ = load_npy_files(train_folder)
test_images, test_labels, test_file_paths = load_npy_files(test_folder)
...
for epoch in epoch_values:
    ...
    models = []
    for i in range(3):
        model = create_keras_model_3(train_images_flattened, train_labels)
        history = model.fit(train_images_flattened, train_labels, epochs=epoch, batch_size=64,
...
if len(stacked_predictions.shape) == 1:
    ...
    metrics = {
        ...
        'AUC Score': auc_score,
        'Loss': loss,
        'Epoch Training Time': time_taken_stack
    }
    metrics_list.append(metrics)
    ...
    plt.savefig(os.path.join(result_folder, 'stacked_model_roc_curve.png'))
    plt.close()
    print('ROC curve saved to:', os.path.join(result_folder, 'stacked_model_roc_curve.png'))

    # Save the metrics to a CSV file
    metrics_df = pd.DataFrame(metrics_list)
    metrics_csv_path = os.path.join(result_folder, 'metrics_summary.csv')
    ...
    plt.xlabel('Predicted Labels')
    plt.ylabel('True Labels')
    plt.savefig(os.path.join(result_folder, 'stacked_model_confusion_matrix.png'))
    plt.close()
    print('Confusion matrix saved to:', os.path.join(result_folder, 'stacked_model_confusion_matrix.png'))
